...
import io.pjmadden.epldashboard.model.Team;
...
public class JobCompletionNotificationListener extends JobExecutionListenerSupport {
    ...
    // EntityManager is the JPA way of interacting with the database
    ...
    public void afterJob(JobExecution jobExecution) {
        if (jobExecution.getStatus() == BatchStatus.COMPLETED) {
           ...
           em.createQuery("select m.homeTeam, count(*) from Match m group by m.homeTeam", Object[].class)
            ...
            em.createQuery("select m.fullTimeResult, count(*) from Match m group by m.fullTimeResult", Object[].class)
            .getResultList()
            .stream()
            .forEach(e -> {
                Team team = teamData.get((String) e[0]);
               if (team != null) team.setTotalMatchesWon((long) e[1]);
            });
